...
const mdxData = {
  ...
  'funktioner/andengradspolynomier': {
    overview: [
      ...
      'Bestemmelse af parablens toppunkt ved hjælp af toppunktsformlen: $T = (-\\frac{b}{2a}, -\\frac{d}{4a})$.'
    ],
  ...
  'infinitesimalregning/optimering-og-monotoni': {
    ...
    motivation: 'Optimering styrer verden. Virksomheder bruger monotonianalyse til at finde den helt præcise produktion, der giver den maksimale profit, og ingeniører bruger det til at designe emballage med mindst muligt spildmateriale.'
  },
  ...
  'matematik-a/avanceret-infinitesimalregning/integrationsregneregler': {
    overview: [
      'Forståelse af det ubestemte integral $\\int f(x) dx$ som bestemmelse af stamfunktioner.',
  ...
  'tal-og-algebra/ligninger': {
    overview: [
      ...
      'Metoder til at isolere en bestemt variabel i en formel.',
  ...
  'tal-og-algebra/regningsarter': {
    ...
    motivation: 'Regnehierarkiet er selve fundamentet for al matematisk logik og computerkodning. Uden disse spilleregler ville computere og lommeregnere ikke kunne udføre en eneste entydig eller korrekt beregning.'
  }
};
...
files.forEach(filePath => {
  ...
  // Check if box is already injected to prevent double injection
  if (content.includes('bg-gradient-to-r from-purple-900/40 to-blue-900/40')) {
    console.log(`Skipping ${relPath} (already processed)`);
    return;
  }

  // Generate the visual Overview & Motivation Box component markup
  const boxHtml = `
